...
import { IconPower, IconClock, IconActivity, IconDownload, IconRefresh, IconTerminal, IconSettings, IconGroup, IconBook } from '../components/icons'
...
export default function StatusPage({ status, onRefresh, onNavigate }: StatusPageProps) {
...
function QuickLink({ title, desc, icon, onClick }: {
    title: string
    desc: string
    ...
    return (
        <button
            type="button"
            className="text-left rounded-xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-white/[0.03] px-4 py-3 hover:bg-gray-100/60 dark:hover:bg-white/[0.06] transition-colors"
            onClick={onClick}
        >
            <div className="flex items-center justify-between">
                <div className="text-sm font-medium text-gray-800 dark:text-gray-200">{title}</div>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center bg-white dark:bg-[#1e1e20] border border-gray-200 dark:border-gray-800 text-gray-600 dark:text-gray-300">
                    {icon}
                </div>
            </div>
